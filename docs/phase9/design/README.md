...
## P9-4 公司研报与个股分析拆分（issue #11）🟡 待定稿门

| 文档 | 状态 | 定稿日期 | 备注 |
|---|---|---|---|
| [company-analysis-split.md](./company-analysis-split.md) | 🟡 **待定稿门** | 2026-09-17 | 按**功能**拆分：**公司研报**（季频文档，财务/估值/行业质地，`/reports`）vs **个股分析**（日频速览，行情/量价/换手/形态，`/research`）。核心发现：量价是风险分析与评分卡的承重墙（§2），故公司研报要保留风险章须**改造风险引擎**（量价规则可选化，§4.2）+ 基本面 Evidence 登记（§4.2.1）。分阶段 P9-4a 风险引擎 / P9-4b 章节解耦 / P9-4c 新 profile / P9-4d 前端。**零 schema**，`complete-stock` 保留为兼容档案 |
