...
- **自选（`entities.status`）**：`watch` = 在自选 / `active` = 库中有不在自选 / `archived` = 曾自选已移出（保留历史/深研/笔记）。自选首页数据源 `GET /api/v1/watchlist`；引入/移出**主经 `watch-sync` 常驻服务自动同步**（`cmd/watch-sync`，每日 3 次拉同花顺「我的自选」，profile 见 `docs/phase11/design/watchlist-sync.md`），**截图镜像为备用通路**（`/trades` 页截图导入 `kind='watchlist'`，服务端 diff add/keep/remove）；仍**无手动星标**。⚠️ **加入价/加入日不在 `entities.detail`**（每轮被 entity-build 覆盖），在 `watchlist_entries`（迁移 0020）。⚠️ 同花顺凭据 `ths_account`/`ths_password`/`ths_cookie` 在 `app_config`，页面写入默认拒（需 `PIKS_ALLOW_THS_CRED_UI=1`，防 #78 未鉴权下被覆盖锁死）。⚠️ **同花顺 XML 一律声明 `encoding="GB2312"`**（Go `encoding/xml` 只认 UTF-8，裸 `xml.NewDecoder` 报 `CharsetReader is nil`）——`internal/ths` 内解析**必须**走 `parse.go` 的 `newXMLDecoder()`（挂 GB18030），直接 `xml.NewDecoder`/`xml.Unmarshal` 会重现此坑。
...
- 编排在 dev 侧：`./scripts/deploy.sh`（**四镜像**分别 build → 按 tag 跳过/传输 → compose 同步 → `up postgres` → `migrate` → `up web research collector hot-topic watch-sync` → `up gateway` → 落 stack 清单）。
- **四镜像**（2026-09-20 容器拆分，issue #47）：`piks-gateway`（纯 nginx，唯一对外）/ `piks-web`（纯 Go API，无 python3）/ `piks-research`（Python 运行时 + 深研队列 worker，常驻）/ `piks-tools`（**13** 管线命令，profile=run；常驻的 `collector`/`hot-topic`/`watch-sync` 服务**复用**它）。单 Dockerfile 多 `--target`。
- **各镜像版本 = 自己输入的 git tree hash**（非 HEAD hash）：改前端不动 web/tools/research 的 tag → 它们既不重建也不传输。tag 形如 `v0.0.0-<该镜像短hash>`。⚠️ `TAG_TOOLS` 取 **13 个命令依赖闭包的并集**（`go_deps_hash_all`），不是只 hash `migrate` —— 否则改 `internal/collector` 不动 tag，生产跑旧像。**新增管线命令须三处同步**：`Dockerfile`（tools 段逐条 `COPY` 新二进制）、`scripts/check-image-topology.sh`（正向清单）、`deploy.sh` 的 `TAG_TOOLS` 参数加该命令名。
- **顺序是硬约束**：`migrate` 必须先于 `web`（`cmd/web/main.go` 读 `app_config`，缺表即 fatal 崩溃循环）；`gateway` 必须最后。`hot-topic` 依赖迁移 0018 的 `hot_topic_items` 表、`watch-sync` 依赖 0020 的 `watchlist_entries` 表，故均排在 `migrate` 之后。
- **调度形态（issue #83 P-5）**：crontab **4 条** —— `pipeline.sh early`（`*/10 9-11 * * 1-5`，重试窗）/ `pipeline.sh late`（`*/10 18-22 * * 1-5`）/ `cleanup.sh`（`0 2 * * 7`）/ `backup.sh`（`59 23 * * 1-5`）。`pipeline.sh` 收 `early|late` 档（**未知档 exit 2**），**按档**闸门/记账（`-$STAGE` 后缀）/`flock`；步骤分 **COMMON**（采集+抽取+合并，两档）+ **EOD**（收盘产物 `daily-review`/`market-state`/`quote-collector` 等，**仅 late** —— 早档跑会为「尚未发生的当日」出报告）。⚠️ **realtime 档不在 cron**：盘中轮询由**常驻 `collector -interval 3m` 服务**承担（与 pipeline 不同进程，天然不被日锁阻断）；`origin_kind='realtime'` 保留未启用。改 `pipeline.sh` 须同改 `docs/架构总览.md` §8.1。
- **保留期纪律（issue #83 P-5）**：`cmd/cleanup`（默认 `-days 28` + `-dry-run`）**只清**「到期 **且** 无 `events.raw_document_id` 引用 **且** 非任何行的 `canonical_id` 目标」的 raw 行 —— **已抽取行永久保留**（事件溯源/抽屉来源/`cluster_sources` 消费它）。⚠️ **「28 天」不是「全体 raw 的 28 天」**；`scripts/cleanup.sh` 闸门 = 周日 **且** 距上次成功 ≥28d（`logs/cleanup.last` **仅成功后更新**）+ `flock`。
