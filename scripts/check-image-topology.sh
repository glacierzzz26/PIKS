...
#   2. tools 只含 9 个管线命令 —— 不该出现 web/research-*/probe
...
# ── 4. tools 只含 9 个管线命令,不含 web/research-*/probe ─────────────────
tools_body="$(section tools)"
if echo "$tools_body" | grep -qE 'COPY --from=build /out/bin/? /app/bin/?$'; then
  die "tools 用整目录 COPY /out/bin/ —— 会把 web/research-*/probe 一并塞入(应逐条列 9 个管线命令)"
fi
...
# 正向:9 个管线命令应一个不少。
for c in migrate collector worker cluster quote-collector entity-build market-state daily-review reconcile; do
  if ! echo "$tools_body" | grep -qE "COPY --from=build /out/bin/${c}\b"; then
    die "tools 段缺管线命令: $c"
  fi
done
...
if [ "$fail" -eq 0 ]; then
  echo "✓ 镜像拓扑检查通过:gateway(nginx)/ web(纯 Go)/ tools(9 管线命令)/ research(Python)"
fi
exit "$fail"
