...
# 股票代码：6 位数字（不带前后数字）。
#
# ⚠️ 必须排除**小数的整数部分**：`695704.0` 的 `695704` 形如 6 位码，旧写法
# `(?<!\d)\d{6}(?!\d)` 会把它整段掩成 `CODE.0`，残留的 `.0` 被当作数据数字扫描
# （`0` 不在卡里）→ GDP/亿元级读数一律 lint 必挂。
# 曾实测：生产 `macro:cn_gdp` 报告 5 个问题全由此产生，context 一律形如
# 「水平为CODE.0亿元」。个股报告只要 LLM 写 6 位小数（如 `2562715.4手`）同挂。
# 判据：后面紧跟「小数点 + 数字」即小数的整数部分，不是代码，不掩。
# 只加 `(?=\.\d)` —— 纯 6 位码（`000560`）与「代码+句点」（`000560.`）仍照旧掩码。
_CODE_RE = re.compile(r"(?<!\d)\d{6}(?!\d)(?!\.\d)")
# 表格分隔线与 markdown 结构
