...
/**
 * 原生 SVG 力导向模拟（自 ForceGraph 抽出，规范第 5 条：组件 <150 行）。
 * 渲染走 ref 直改 DOM 属性，不经 React 状态，保证 60fps。
 * 视图（SVG 元素 + ref 回调）留在 ForceGraph，模拟与交互全在本 hook。
 */
...
export function useForceSim({ entities, relationships, focusId, onSelect }: Options) {
  ...
  useEffect(() => {
    ...
    // 注意:不能再清空 lineEls/nodeEls/labelEls —— 它们由视图 ref 回调在 React commit
    // 阶段填充,effect 在 commit 之后执行,clear() 会清空刚填好的 Map,导致节点永停
    // (0,0)。数据变化时由 ref 回调的 null 分支同步增删,天然对齐当前实体集。
  ...
  return { svgRef, gRef, lineEls, nodeEls, labelEls };
}
